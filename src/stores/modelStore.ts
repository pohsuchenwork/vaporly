...
export const useModelStore = create<ModelsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    // The backend no longer exposes set_active_model: the single catalog
    // model IS the selection (the settings default). Kept as a local no-op so
    // onboarding still flows; M6 rebuilds onboarding for the fixed-model world.
