  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <button
        onClick={togglePlay}
        disabled={isLoading}
        className="inline-grid place-items-center min-h-8 min-w-8 shrink-0 transition-colors cursor-pointer text-ink hover:text-accent-ink disabled:opacity-60 disabled:cursor-not-allowed focus-ring rounded-control"
      ...
      <div className="flex-1 flex items-center gap-2">
        <span className="text-xs text-ink-muted min-w-[30px] tabular-nums">
        ...
        <input
          ...
          className={`flex-1 h-1 rounded-full appearance-none cursor-pointer focus-ring ${progressPercent >= 99.5 ? "[&::-webkit-slider-thumb]:translate-x-0.5 [&::-moz-range-thumb]:translate-x-0.5" : ""}`}
          style={{
            background: `linear-gradient(to right, var(--color-accent) 0%, var(--color-accent) ${progressPercent}%, var(--color-surface-well) ${progressPercent}%, var(--color-surface-well) 100%)`,
          }}
        />
