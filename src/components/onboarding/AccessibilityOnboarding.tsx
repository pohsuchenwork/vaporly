...
const AccessibilityOnboarding: React.FC<AccessibilityOnboardingProps> = ({
  ...
  // Escape hatch: after a grant has been "waiting" a while (e.g. a stale TCC
  // entry or a re-signed rebuild that macOS no longer trusts), surface a manual
  // recheck + a Continue-anyway so the user is never trapped on a dead spinner.
  const escapeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [showEscape, setShowEscape] = useState(false);

  const armEscape = useCallback(() => {
    if (escapeTimerRef.current) return;
    escapeTimerRef.current = setTimeout(() => setShowEscape(true), 8000);
  }, []);
  ...
  // Manual recheck, runs the same check the poll does, once, on demand.
  const checkNow = useCallback(async () => {
    try {
      ...
      setPermissions((prev) => ({
        ...prev,
        accessibility: acc ? "granted" : prev.accessibility,
        microphone: mic ? "granted" : prev.microphone,
      }));
      if (acc) {
        Promise.all([
          commands.initializeEnigo(),
      ...
      if (acc && mic) await completeOnboarding();
    } catch (e) {
      console.error("Manual recheck failed:", e);
    }
  }, [permissionPlatform, hasWindowsMicrophoneAccess, completeOnboarding]);
  ...
  if (isChecking) {
    return (
      <div className="h-screen w-screen flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-ink-subtle" />
      </div>
    );
  }
  ...
  if (allGranted) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center gap-4">
        <div className="motion-pop p-4 rounded-full bg-success-tint">
  ...
  return (
    <div className="h-screen w-screen flex flex-col p-6 gap-6 items-center justify-center">
      <div className="motion-rise flex flex-col items-center gap-2">
        <VaporlyWordmark width={200} />
      </div>

      <div className="motion-stagger max-w-md w-full flex flex-col items-center gap-4">
        ...
        {showMicrophonePermission && (
          <Card className="w-full p-4">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-full bg-accent-tint shrink-0">
        ...
        {/* Escape hatch: appears if a grant stays stuck (stale TCC entry / a
            re-signed rebuild macOS no longer trusts). Never leave the user
            trapped, the persistent in-app banner keeps nagging until real. */}
        {showEscape && !allGranted && (
          <div className="w-full flex flex-col items-center gap-2 pt-1 text-center">
            <p className="text-xs text-ink-subtle max-w-sm">
              {t("onboarding.permissions.stuckHint")}
            </p>
            <div className="flex items-center gap-3">
              <Button onClick={checkNow} variant="secondary" size="sm">
                {t("onboarding.permissions.recheck")}
              </Button>
