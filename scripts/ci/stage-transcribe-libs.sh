...
#
# On Linux/Windows, Vaporly builds transcribe-cpp in its `dynamic-backends`
...
# them. This script copies them, as a co-located set, next to where the `vaporly`
# binary will find them at runtime:
#   - the loader resolves `libtranscribe` via the $ORIGIN-relative rpath baked
#     into `vaporly` (see src-tauri/build.rs);
