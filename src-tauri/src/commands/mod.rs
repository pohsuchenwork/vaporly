...
pub fn reset_all_settings(app: AppHandle) -> Result<(), String> {
    ...
    fresh.onboarding_completed = current.onboarding_completed;
    write_settings(&app, fresh);
    // Re-register shortcuts against the restored default bindings.
    crate::shortcut::init_shortcuts(&app);
    // The engine may need to follow the restored provider defaults.
    if let Some(engine) =
