...
pub fn play_feedback_sound(app: &AppHandle, sound_type: SoundType) {
    ...
    if let Some(path) = resolve_sound_path(app, &settings, sound_type) {
        play_sound_async(app, path, None);
    }
}
...
pub fn play_test_sound(app: &AppHandle, sound_type: SoundType) {
    let settings = settings::get_settings(app);
    if let Some(path) = resolve_sound_path(app, &settings, sound_type) {
        // Play on a dedicated thread like real feedback does; blocking here ran
        // sink.sleep_until_end() on the async command worker. Preview at an
        // audible floor: a persisted volume of 0 must not make the preview
        // button look broken (real feedback still honors the true volume).
        let volume = settings.audio_feedback_volume.max(0.3);
        play_sound_async(app, path, Some(volume));
    }
}

fn play_sound_async(app: &AppHandle, path: PathBuf, volume_override: Option<f32>) {
    let app_handle = app.clone();
    thread::spawn(move || {
        if let Err(e) = play_sound_at_path(&app_handle, path.as_path(), volume_override) {
            error!("Failed to play sound '{}': {}", path.display(), e);
        }
    });
}

fn play_sound_blocking(app: &AppHandle, path: &Path, volume_override: Option<f32>) {
    if let Err(e) = play_sound_at_path(app, path, volume_override) {
        error!("Failed to play sound '{}': {}", path.display(), e);
    }
}
...
fn play_sound_at_path(
    ...
    let settings = settings::get_settings(app);
    let volume = volume_override.unwrap_or(settings.audio_feedback_volume);
