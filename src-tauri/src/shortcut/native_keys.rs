...
use vaporly_keys::{Hotkey, HotkeyId, HotkeyManager, HotkeyState, KeyboardListener};
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("vaporly-keys-event", &frontend_event) {
...
fn modifiers_to_strings(modifiers: vaporly_keys::Modifiers) -> Vec<String> {
    let mut result = Vec::new();

    if modifiers.contains(vaporly_keys::Modifiers::CTRL) {
        result.push("ctrl".to_string());
    }
    if modifiers.contains(vaporly_keys::Modifiers::OPT) {
        #[cfg(target_os = "macos")]
        result.push("option".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("alt".to_string());
    }
    if modifiers.contains(vaporly_keys::Modifiers::SHIFT) {
        result.push("shift".to_string());
    }
    if modifiers.contains(vaporly_keys::Modifiers::CMD) {
        #[cfg(target_os = "macos")]
        result.push("command".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("super".to_string());
    }
    if modifiers.contains(vaporly_keys::Modifiers::FN) {
...
/// Validate a shortcut string for the native-keys implementation.
/// Native keys is more permissive: allows modifier-only combos and the fn key.
...
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    ...
    for (id, default_binding) in default_bindings {
        ...
        // Unbound (empty) bindings simply stay unregistered.
...
pub fn start_native_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    ...
    if settings.keyboard_implementation != settings::KeyboardImplementation::VaporlyNative {
