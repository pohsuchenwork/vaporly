...
use crate::settings;
use crate::tray_i18n::TRAY_STRINGS;
...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/vaporly.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Vaporly v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Vaporly v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, _locale: Option<&str>) {
    ...
    // v2 UI is English-only; the tray follows.
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "vaporly-1.wav".to_string(),
            timestamp: 0,
            saved: false,
