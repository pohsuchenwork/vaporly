...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    if env_flag_enabled("VAPORLY_NO_GTK_LAYER_SHELL")
        || env_flag_enabled("HANDY_NO_GTK_LAYER_SHELL")
    {
        debug!("Skipping GTK layer shell init (VAPORLY_NO_GTK_LAYER_SHELL is enabled)");
        return false;
    }
...
/// Bumped on every overlay show; result flashes capture it so their delayed
/// auto-hide never dismisses an overlay that a newer action put up.
...
/// Hands-free latch indicator on the overlay (lock glyph while latched).
pub fn set_overlay_hands_free(app_handle: &AppHandle, latched: bool) {
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        let _ = overlay_window.emit("hands-free-changed", latched);
    }
}

/// One-minute warning before the hands-free session cap stops dictation.
pub fn emit_hands_free_warning(app_handle: &AppHandle) {
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        let _ = overlay_window.emit("hands-free-warning", 60u64);
    }
}
...
/// Brief result flash after the pipeline finishes, then auto-hide, unless a
/// newer overlay took the stage in the meantime (generation guard).
fn flash_result_overlay(app_handle: &AppHandle, state: &'static str, visible_ms: u64) {
    show_overlay_state(app_handle, state);
    let generation = OVERLAY_GENERATION.load(std::sync::atomic::Ordering::Relaxed);
    let app_handle = app_handle.clone();
    std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(visible_ms));
        if OVERLAY_GENERATION.load(std::sync::atomic::Ordering::Relaxed) == generation {
            crate::utils::hide_recording_overlay(&app_handle);
        }
    });
}

/// Success tick once the text has been inserted at the cursor.
pub fn flash_inserted_overlay(app_handle: &AppHandle) {
    flash_result_overlay(app_handle, "inserted", 900);
}

/// Brief failure notice when inserting at the cursor failed.
pub fn flash_insert_error_overlay(app_handle: &AppHandle) {
    flash_result_overlay(app_handle, "error", 1600);
}
