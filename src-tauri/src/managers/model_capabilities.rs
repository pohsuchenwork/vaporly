//! Model capability probing, the seam between Vaporly's model UI and "what can
...
/// How confident we are that Vaporly can run a given model, judged from its GGUF
...
/// prober later).
/// Header-probe surface: production callers (the discovery scans) were
/// removed with the fixed-model strip-down; kept, with tests, for the next
/// pre-download metadata consumer.
#[cfg_attr(not(test), allow(dead_code))]
