...
This guide covers how to set up the development environment and build Vaporly from source across different platforms.
...
```bash
git clone git@github.com:pohsuchenwork/vaporly.git
cd vaporly
```
...
The voice activity detector model is required to build and run Vaporly:
...
The raw binary (`src-tauri/target/release/vaporly`) cannot run standalone, it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/vaporly/src-tauri/target/release/bundle/deb/Vaporly_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/vaporly /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Vaporly.desktop /usr/share/applications/
sudo ldconfig
```
...
```bash
sudo cp src-tauri/target/release/vaporly /usr/bin/
...
```
Bundling Vaporly_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Vaporly.AppDir --plugin gtk --output appimage
```
...
this because it builds from a short root such as `D:\a\vaporly`.)
...
Alternatively, clone the repo to a short root (e.g. `C:\vaporly`).
